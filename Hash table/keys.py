class HashTable:                    # will only create the table 
    ...
    def keys(self):
        all_keys = []               # create an empty list where will add our keys
        for i in range(len(self.data_map)):        # we loop through our table
            if self.data_map[i] is not None:       # only loop forward for keys if there is data at that index
                for j in range(len(self.data_map[i])):    # we loop through the big list 
                    all_keys.append(self.data_map[i][j][0])     # we append all keys from the table and inside big list at index 0 ie keys
        return all_keys
