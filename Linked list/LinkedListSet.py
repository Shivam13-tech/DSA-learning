...
class LinkedList:             
    ...
    def set_value(self,index,value):
        temp = self.get(index)                   # here we can set using 2 methods a. normal one commented above or b. where we use method inside a method and pass it an index to get the exact node instead of looping
        if temp is not None:                     # the get method can return none if the index is out of range so we need to cover that edge case
            temp.value = value                   # we set the value and return true if it all worked else return False
            return True
        return False
